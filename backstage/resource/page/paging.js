(function ($) {
    $.fn.paging = function (options) {
        ...
        // }
        if(box.siblings('.pagenation').length <= 0){
            box.html("<div class='pagenation' onselectstart='return false'><div class='pagenum indexpage'>首页</div><div class='pagenum lastpage'><a><上一页</a></div><div class='pageswiperbox'><div class='pageswiper'><div class='pagenum curpage pn'><a>1</a></div></div></div><div class='pagenum nextpage active'><a>下一页></a></div><div class='pagenum totalpage'>共<span></span>页</div><div class='pagenum pagetext'>去第<input type='text' class='pageinput'/>页</div><div class='pagenum pagesubbtn'><a>确定</a></div></div>");
        }
        ...
        if (pagenation.find(".pn").eq(0).hasClass("curpage")) {   // pagenation.find(".pn").first().hasClass("curpage")
            pagenation.find(".lastpage").removeClass("active")
        } else {
            pagenation.find(".lastpage").addClass("active")
        };
        if (pagenation.find(".pn").eq(pagenation.find(".pn").length - 1).hasClass("curpage")) {    // pagenation.find(".pn").last().hasClass("curpage")
            pagenation.find(".nextpage").removeClass("active")
        } else {
            pagenation.find(".nextpage").addClass("active")
        };
        pagenation.find(".pageinput").bind("input propertychange",    //pagenation.find(".pageinput").on("input propertychange",
            function () {
        ...
        pagenation.find(".pn").bind("click",      // pagenation.find(".pn").on("click",
            function () {
                $(this).addClass("curpage");
                $(this).siblings(".pagenation .pn").removeClass("curpage")
            });
        pagenation.find(".pagenum").unbind("click").bind("click",     // pagenation.find(".pagenum").unbind("click").on("click",
            function () {
                if ($(this).hasClass("pn")) {
                    var k = $(this).text();
